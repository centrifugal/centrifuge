...
// KeyedCacheConfig configures the in-memory cache for keyed channels.
...
// DefaultKeyedCacheConfig returns the default cache configuration.
func DefaultKeyedCacheConfig() KeyedCacheConfig {
	return KeyedCacheConfig{
		MaxChannels:        10000,
...
type ChannelLoader func(ctx context.Context, ch string, opts KeyedChannelOptions) (snapshot []*Publication, stream []*Publication, pos StreamPosition, err error)

// KeyedCache provides in-memory caching for keyed channel data.
...
type KeyedCache interface {
	...
	EnsureLoaded(ctx context.Context, ch string, opts KeyedChannelOptions, loader ChannelLoader) error
	...
	// GetSnapshot retrieves snapshot data from cache.
	GetSnapshot(ch string, opts KeyedReadSnapshotOptions) ([]*Publication, StreamPosition, string, error)

	// GetStream retrieves stream data from cache.
	GetStream(ch string, opts KeyedReadStreamOptions) ([]*Publication, StreamPosition, error)

	// GetStats returns statistics about a cached channel.
	GetStats(ch string) (KeyedStats, error)
...
type cachedChannel struct {
	...
	options         KeyedChannelOptions // Per-channel options for TTL/size
}
...
// EnsureLoaded ensures the channel is loaded into cache.
func (c *keyedCacheImpl) EnsureLoaded(ctx context.Context, ch string, opts KeyedChannelOptions, loader ChannelLoader) error {
...
// Evict removes a channel from the cache.
func (c *keyedCacheImpl) Evict(ch string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.evictLocked(ch)
}
...
// IsLoaded returns true if the channel is currently in cache.
func (c *keyedCacheImpl) IsLoaded(ch string) bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.loaded[ch]
}
...
// LoadedChannels returns a list of all currently loaded channels.
func (c *keyedCacheImpl) LoadedChannels() []string {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
// GetSnapshot retrieves snapshot data from cache.
func (c *keyedCacheImpl) GetSnapshot(ch string, opts KeyedReadSnapshotOptions) ([]*Publication, StreamPosition, string, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// GetStream retrieves stream data from cache.
func (c *keyedCacheImpl) GetStream(ch string, opts KeyedReadStreamOptions) ([]*Publication, StreamPosition, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
// GetStats returns statistics about a cached channel.
func (c *keyedCacheImpl) GetStats(ch string) (KeyedStats, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
// GetEpoch returns the epoch for a cached channel, or empty string if not loaded.
func (c *keyedCacheImpl) GetEpoch(ch string) string {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *keyedCacheImpl) GetPosition(ch string) StreamPosition {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
// ApplySnapshot replaces the entire snapshot for a channel.
func (c *keyedCacheImpl) ApplySnapshot(ch string, pubs []*Publication, pos StreamPosition, opts *KeyedChannelOptions) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	var channelOpts KeyedChannelOptions
	if opts != nil {
		channelOpts = *opts
	} else if existing, ok := c.channels[ch]; ok {
		channelOpts = existing.options
	} else {
		channelOpts = DefaultKeyedChannelOptions()
	}
...
func (c *keyedCacheImpl) Close() error {
	c.closeOnce.Do(func() {
		close(c.closeCh)
	})
	return nil
...
func (c *keyedCacheImpl) maybeEvictLocked() {
...
func (c *keyedCacheImpl) runIdleEviction() {
...
func (c *keyedCacheImpl) evictIdleChannels() {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *keyedCacheImpl) makeOrderedCursor(score int64, key string) string {
	return strconv.FormatInt(score, 10) + "\x00" + key
}

// parseOrderedCursor parses an ordered cursor into score and key.
func (c *keyedCacheImpl) parseOrderedCursor(cursor string) (int64, string) {
...
func (c *keyedCacheImpl) findUnorderedCursorPosition(sortedKeys []string, cursor string) int {
...
func (c *keyedCacheImpl) findOrderedCursorPosition(channel *cachedChannel, cursor string) int {
...
// keyedCacheWithEviction extends keyedCacheImpl with priority-based eviction.
type keyedCacheWithEviction struct {
	*keyedCacheImpl
	accessQueue priority.Queue
	accessMap   map[string]int64 // ch -> last access timestamp
}

// newKeyedCacheWithEviction creates a cache with LRU eviction support.
func newKeyedCacheWithEviction(conf KeyedCacheConfig) *keyedCacheWithEviction {
	c := &keyedCacheWithEviction{
		keyedCacheImpl: newKeyedCache(conf),
...
func (c *keyedCacheWithEviction) updateAccess(ch string) {
