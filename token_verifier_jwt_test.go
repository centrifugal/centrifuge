...
func Test_tokenVerifierJWT_Valid(t *testing.T) {
	verifier := NewTokenVerifierJWT(TokenVerifierConfig{"secret", nil})
...
func Test_tokenVerifierJWT_DisabledAlgorithm(t *testing.T) {
	verifier := NewTokenVerifierJWT(TokenVerifierConfig{"", nil})
	_, err := verifier.VerifyConnectToken(jwtExpired)
	require.Error(t, err)
...
func Test_tokenVerifierJWT_VerifyConnectToken(t *testing.T) {
	...
	verifierJWT := NewTokenVerifierJWT(TokenVerifierConfig{"secret", pubKey})
	_time := time.Now()
	tests := []struct {
		name     string
...
func BenchmarkConnectTokenVerify_Valid(b *testing.B) {
	verifierJWT := NewTokenVerifierJWT(TokenVerifierConfig{"secret", nil})
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
