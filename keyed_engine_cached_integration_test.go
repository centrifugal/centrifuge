...
// setupTwoNodeCachedRedisWithPubSub creates two CachedKeyedEngine instances with long sync interval
// to ensure tests rely on PUB/SUB propagation, not periodic sync.
func setupTwoNodeCachedRedisWithPubSub(t *testing.T) (node1 *Node, cached1 *CachedKeyedEngine, node2 *Node, cached2 *CachedKeyedEngine, cleanup func()) {
	// Node 1
	node1, _ = New(Config{})
	redisConf := testSingleRedisConf(6379)
	shard1, err := NewRedisShard(node1, redisConf)
...
// TestCachedKeyedEngine_TwoNodes_PubSub tests real PUB/SUB propagation between nodes.
// Uses Subscribe to set up PUB/SUB channels and verifies real-time updates.
func TestCachedKeyedEngine_TwoNodes_PubSub(t *testing.T) {
...
// TestCachedKeyedEngine_TwoNodes_UpdateSameKeyPubSub tests both nodes updating the same key
// and verifies both caches converge to the same final value via real PUB/SUB.
func TestCachedKeyedEngine_TwoNodes_UpdateSameKeyPubSub(t *testing.T) {
	_, cached1, _, cached2, cleanup := setupTwoNodeCachedRedisWithPubSub(t)
	defer cleanup()
...
// TestCachedKeyedEngine_TwoNodes_GapFillingPubSub tests that when node2 subscribes after
// node1 has published, it gets all data and then receives real-time updates via PUB/SUB.
func TestCachedKeyedEngine_TwoNodes_GapFillingPubSub(t *testing.T) {
	_, cached1, _, cached2, cleanup := setupTwoNodeCachedRedisWithPubSub(t)
	defer cleanup()
...
// TestCachedKeyedEngine_TwoNodes_UnpublishPubSub tests that unpublish (removal) propagates
// correctly to both nodes via real PUB/SUB (not periodic sync).
func TestCachedKeyedEngine_TwoNodes_UnpublishPubSub(t *testing.T) {
	_, cached1, _, cached2, cleanup := setupTwoNodeCachedRedisWithPubSub(t)
	defer cleanup()
...
// TestCachedKeyedEngine_TwoNodes_ConcurrentPublishPubSub tests concurrent publications
// from both nodes using real PUB/SUB propagation (not periodic sync).
func TestCachedKeyedEngine_TwoNodes_ConcurrentPublishPubSub(t *testing.T) {
	_, cached1, _, cached2, cleanup := setupTwoNodeCachedRedisWithPubSub(t)
	defer cleanup()
...
// TestCachedKeyedEngine_TwoNodes_MissedPubSubMessages tests that when a node misses
...
func TestCachedKeyedEngine_TwoNodes_MissedPubSubMessages(t *testing.T) {
	...
	require.NoError(t, err)
	silentBackend, err := NewRedisKeyedEngine(node3, RedisKeyedEngineConfig{
	...
	pubs1, _, _, _ := cached1.ReadSnapshot(ctx, channel, KeyedReadSnapshotOptions{Limit: 100})
	require.Len(t, pubs1, 1)
	pubs2, pos2, _, _ := cached2.ReadSnapshot(ctx, channel, KeyedReadSnapshotOptions{Limit: 100})
	require.Len(t, pubs2, 1)
	...
	for i := 2; i <= 4; i++ {
		_, err = silentBackend.Publish(ctx, channel, fmt.Sprintf("key%d", i), KeyedPublishOptions{
			Data:       []byte(fmt.Sprintf("data%d", i)),
			StreamSize: 100,
			StreamTTL:  300 * time.Second,
		})
	...
	_, err = cached1.Publish(ctx, channel, "key5", KeyedPublishOptions{
		Data:       []byte("data5"),
		StreamSize: 100,
		StreamTTL:  300 * time.Second,
	})
...
// TestCachedKeyedEngine_TwoNodes_StreamOrderConsistency verifies that the stream
// order is identical on both nodes after publications from both sides.
func TestCachedKeyedEngine_TwoNodes_StreamOrderConsistency(t *testing.T) {
	_, cached1, _, cached2, cleanup := setupTwoNodeCachedRedisWithPubSub(t)
	defer cleanup()
	...
	for i := 0; i < 10; i++ {
		_, err = cached1.Publish(ctx, channel, fmt.Sprintf("n1_key%d", i), KeyedPublishOptions{
		...
		_, err = cached2.Publish(ctx, channel, fmt.Sprintf("n2_key%d", i), KeyedPublishOptions{
	...
	stream1, pos1, err := cached1.ReadStream(ctx, channel, KeyedReadStreamOptions{
		Filter: StreamFilter{Limit: -1},
	})
	require.NoError(t, err)

	stream2, pos2, err := cached2.ReadStream(ctx, channel, KeyedReadStreamOptions{
		Filter: StreamFilter{Limit: -1},
	})
	require.NoError(t, err)
...
// TestCachedKeyedEngine_TwoNodes_HighConcurrencyPubSub stress tests with high concurrency
// using real PUB/SUB propagation (not periodic sync).
func TestCachedKeyedEngine_TwoNodes_HighConcurrencyPubSub(t *testing.T) {
	_, cached1, _, cached2, cleanup := setupTwoNodeCachedRedisWithPubSub(t)
	defer cleanup()
