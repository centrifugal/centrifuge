...
// CI runs the integration suite once and gets all three variants.
var mapBrokerVariants = []mapBrokerVariant{
	{"rd_single", false, false},
	{"rd_cluster", true, false},
	{"rd_cluster_ng", true, true},
}
...
func (f redisMapBrokerFactory) makeCustom(tb testing.TB, n *Node, h BrokerEventHandler, tweak func(*RedisMapBrokerConfig)) *RedisMapBroker {
	...
	if f.variant.UseCluster {
		if f.variant.NodeGrouped {
			brokerConf.NumShardedPubSubPartitions = 16
			brokerConf.GroupShardedPubSubByNode = true
		} else {
